...
<body>
	...
	<?php include "menu/me_administrativo.php"; ?>
...
<section id="contact" class="home-section bg-white">
	<div class="container">
		<div class="row">    
			<div class="col-md-offset-2 col-md-8">
				<h1>Pedidos de Reserva de Valor</h1>
				<p><?php if(isset($mensagem)){ echo $mensagem; }?></p>
			</div>
		</div>
				<div class="row">  
		...
		<div class="container">
			...
			<div class="table-responsive">
				<table class="table table-striped">
					...
					<tbody>
						<?php 
							$sql_seleciona = "SELECT * FROM sc_reserva WHERE publicado = '1' ORDER BY id DESC";
							$reserva = $wpdb->get_results($sql_seleciona,ARRAY_A);



						for($i = 0; $i < count($reserva); $i++){
							$userwp = get_userdata($reserva[$i]['responsavel01']);
					//var_dump($pedido);
							?>
							<tr>
								<td><?php echo $reserva[$i]['n_reserva_cco']; ?></td>
								<td><?php echo $reserva[$i]['titulo']; ?></td>
								<td><?php echo dinheiroParaBr($reserva[$i]['valor']); ?></td>
								<td><?php echo $userwp->first_name." ".$userwp->last_name; ?></td>
								<td><?php if($reserva[$i]['validade'] != "0000-00-00"){ echo exibirDataBr($reserva[$i]['validade']);} ?></td>
							
								<td><form method="POST" action="?p=editar_pedido" class="form-horizontal" role="form" target="_blank">
									<input type="hidden" name="editar_pedido" value="<?php echo $peds[$i]['idPedidoContratacao']; ?>" />
									<input type="submit" class="btn btn-theme btn-sm btn-block" value="Editar Pedido">
								</form>
									</td>
