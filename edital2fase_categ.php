...
<main class="col-sm-9 offset-sm-3 col-md-10 offset-md-2 pt-3">
    ...
    <p>Para ter acesso aos detalhes dos projetos, é necessário que esteja logado no CulturAZ e que faça parte da equipe de pareceristas.
        <a href="http://culturaz.santoandre.sp.gov.br/autenticacao/" target="_blanck">Clique para logar</a></p>
    <p>
        <?php
        ...
        for($i = 0; $i < count($res_fil); $i++){
            if($f != $res_fil[$i]['filtro']){
                echo "<a href='edital2fase_categ.php?edital=".$_GET['edital']."&filtro=".$res_fil[$i]['filtro']."' >".$res_fil[$i]['filtro']."</a> |  ";
            }else{
                echo $res_fil[$i]['filtro']." | ";
            }
        }
        echo "<a href='edital2fase_categ.php?edital=".$_GET['edital']."&revisao=1' >Revisão</a>";
        ...
        <table class="table table-striped">
            ...
            for($i = 0; $i < count($res); $i++){
                ...
                <tr>
                    ...
                    <td><?php echo str_replace('Categoria',"",$res_json['Categoria']); ?></td>
                    <td><?php echo $res_json['Determine a área principal de enquadramento da proposta']; ?></td>
                    <td><?php echo $res_json['Valor (em Reais)']; ?></td>
                    <td><?php echo retornaNota2Fase($json['inscricao'],577); ?></td>
